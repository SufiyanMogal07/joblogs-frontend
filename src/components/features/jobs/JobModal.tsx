  ...
  const JobModal = ({
    ...
    const {
      ...
      defaultValues: {
        ...
        source: "" as JobApplication["source"],
        priority: false,
        appliedAt: undefined,
        notes: ""
      }
    ...
    useEffect(() => {
      ...
      if (jobStatusVal === "draft") {
        lastAppliedDate.current = watch("appliedAt") ?? undefined;
        setValue("appliedAt", undefined);
        clearErrors("appliedAt");
      }
    ...
    return (
      ...
      >
        ...
        <form
          ...
          className="grid grid-cols-1 lg:grid-cols-2 gap-y-3 gap-x-6 lg:gap-x-10 overflow-y-auto px-4 sm:px-5 md:px-6 lg:px-10 py-5 sm:py-6 flex-1 min-h-0"
        >
          ...
          <div className="flex-column">
            ...
            <div
              className={`form-input-container cursor-pointer select-none transition-colors duration-200 ${
                watch("priority") ? "ring-1 ring-yellow-500/40 bg-gray-800/80" : ""
              }`}
          ...
          <div onClick={() => setFocus("appliedAt")} className="flex-column">
            <label className="form-label" htmlFor="">
              Date Applied{" "}
              {!isDisabled && <span className="text-red-500 text-sm">*</span>}
            </label>
            <div
